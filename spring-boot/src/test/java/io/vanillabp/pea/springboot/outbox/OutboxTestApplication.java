 ...
 * Process-Engine-API adapter (mock-backed), a JPA aggregate and the JDBC phase-two
 * outbox the platform serves it with, so {@code ProcessService#startWorkflow} exercises
 * the real two-phase start: phase one ({@code PREFLIGHT_CHECK}) inside the transaction,
 * phase two ({@code SYNC}) after commit via the outbox.
 */
